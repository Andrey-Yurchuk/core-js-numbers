...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lenghtFirstVector = Math.sqrt(x1 * x1 + y1 * y1);
  const lenghtSecondVector = Math.sqrt(x2 * x2 + y2 * y2);

  const cosinus = scalarProduct / (lenghtFirstVector * lenghtSecondVector);
